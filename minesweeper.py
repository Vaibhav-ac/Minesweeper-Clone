...
class boardClass(object):
        ...
        for y in range(0, self.boardSize):
            ...
            for x in range(0, self.boardSize):
                if self.board[x][y].mine and self.board[x][y].selected:
                    ...
                    # It reduces the extra space created by -1
    ...
    def addMine(self, x, y):
        ...
        # This adds mine to the spot by changing the spot value to -1 and
        # mine as true
        for i in range(x-1, x+2):
            if i >= 0 and i < self.boardSize:
              # This checks the cordinates remain inbound while we are 
              # adding the values in the adjacent side of the mine
    ...
    def makeMove(self, x, y):
        ...
        # It marks that spot as selected and reduces the number of 
        # available selectablespots
        if self.board[x][y].value == -1:
            return False
            # It checks whether the spot has mine or not
        if self.board[x][y].value == 0:
          # It reduces recursion to check for all the adjacent spots with 
          # value = 0
            for i in range(x-1, x+2):
                if i >= 0 and i < self.boardSize:
          ...
          # It checks whether the spot has non-zero and non-negative value 
          # or not
            return True
